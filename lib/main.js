...
async function run() {
  try {
    ...
    const existingRelease = releases.filter((er) => er.tag_name == release_id) 
    ...
    if (existingRelease.length == 0) {
        ...
        const { data: { upload_url: url } } = existingRelease
    }

    const { data: { upload_url: url } } = await octokit.repos.getReleaseByTag({owner, repo, tag: release_id})
